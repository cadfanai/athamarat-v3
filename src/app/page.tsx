...
<span className="text-orange-400">together.</span>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-16">
              ...
              <Link href="/wholesale" className="inline-flex items-center gap-2 px-7 py-4 border-2 border-white/40 text-white font-semibold text-base rounded-xl hover:border-white/70 hover:bg-white/10 transition-all">
                Wholesale Enquiry
              </Link>
            </div>

            {/* Stats */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-px bg-white/10 rounded-2xl overflow-hidden backdrop-blur-sm">
              {STATS.map(s => (
      ...
      <section className="py-16 bg-white">
        <div className="container-main">
          <div className="grid md:grid-cols-2 gap-6">
            <Link href="/shop" className="group relative rounded-2xl overflow-hidden border-2 border-green-100 hover:border-green-500 p-8 transition-all hover:shadow-xl bg-white">
              <div className="text-4xl mb-4">🌾</div>
              <h3 className="font-display text-2xl font-bold text-green-900 mb-2">I'm a Farmer</h3>
              <p className="text-gray-500 mb-6">Buy single packs, starter kits and tools. Clear safe-use guidance included.</p>
            ...
            <Link href="/wholesale" className="group relative rounded-2xl overflow-hidden border-2 border-orange-100 hover:border-orange-500 p-8 transition-all hover:shadow-xl bg-white">
              <div className="text-4xl mb-4">🏢</div>
              <h3 className="font-display text-2xl font-bold text-green-900 mb-2">I'm a Business</h3>
              <p className="text-gray-500 mb-6">Wholesale pricing, bulk orders, proforma invoices and compliance docs for distributors.</p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-10">
            {PROBLEMS.map(p => (
              <Link key={p.id} href={`/shop/by-problem/${p.id}`}
                className="flex flex-col items-center text-center p-5 bg-white rounded-2xl border border-stone-200 hover:border-green-400 hover:shadow-md transition-all group">
      ...
      <section className="relative h-72 overflow-hidden">
        <Image src="/images/athamarat-pic2.png" alt="East African vegetable farm at sunset" fill className="object-cover" />
        <div className="absolute inset-0 bg-green-900/50 flex items-center justify-center">
          <div className="text-center text-white px-4">
            ...
            <p className="text-white/80">Ethiopia · Somalia · Kenya · Djibouti · Eritrea</p>
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-10">
            {SOLUTIONS.map(s => (
              <Link key={s.id} href={`/shop/by-solution/${s.id}`}
                className="flex gap-4 p-6 rounded-2xl border border-stone-200 hover:border-green-400 hover:shadow-lg transition-all group bg-white">
                <span className="text-3xl shrink-0">{s.icon}</span>
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="font-semibold text-gray-900 text-sm group-hover:text-green-800">{s.label}</h3>
                    <span className={s.tier === 'GOLD' ? 'tier-gold' : 'tier-silver'}>{s.tier}</span>
                  </div>
      ...
      <section className="section bg-green-950">
        <div className="container-main">
          ...
          <div className="text-center mt-10">
            <Link href="/shop" className="btn-orange">Browse all products <ArrowRight size={16} /></Link>
          </div>
        </div>
      </section>
      ...
      <section className="section bg-stone-50">
        <div className="container-main">
          ...
          <div className="grid md:grid-cols-2 gap-6 mt-10">
            {BUNDLES.map(b => (
              <div key={b.ref} className="bg-white border border-stone-200 rounded-2xl p-7 hover:shadow-lg transition-all">
                <div className="flex items-start justify-between mb-4">
      ...
      <section className="relative h-80 overflow-hidden">
        <Image src="/images/athamarat-pic3.png" alt="Ethiopian highlands" fill className="object-cover object-center" />
        <div className="absolute inset-0 bg-gradient-to-t from-green-950/80 via-green-900/30 to-transparent flex items-end">
          <div className="container-main pb-12 text-white">
            ...
            <p className="text-white/75 max-w-lg">18+ years supplying farms, institutions and government across one of the world's most dynamic agricultural regions.</p>
          </div>
        </div>
      </section>
        ...
        <div className="container-main">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <p className="section-sub">Regulations are tightening. Export markets demand clean supply chains. We're ahead of the curve.</p>
              <div className="mt-8 space-y-4">
                {[
                  ...
                  <div key={title as string} className="flex gap-4 p-4 rounded-xl border border-stone-200 hover:border-green-200 transition-all">
              ...
              <div className="mt-8">
                <Link href="/why-ethical" className="btn-primary">Our full approach <ArrowRight size={16} /></Link>
              </div>
            </div>
            <div className="relative h-96 rounded-3xl overflow-hidden shadow-2xl">
              <Image src="/images/athamarat-pic4.png" alt="Simien Mountains Ethiopia" fill className="object-cover" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="section bg-stone-50">
        <div className="container-main">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-10">
            {CLIENTS.map(c => (
              <div key={c.name} className="bg-white border border-stone-200 rounded-2xl p-5 hover:shadow-md transition-all">
      ...
      <section className="relative py-24 overflow-hidden">
        <Image src="/images/athamarat-pic1.png" alt="Farm background" fill className="object-cover" />
